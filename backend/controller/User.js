...
export const updateUser = async (req, res) => {
  try {
    const user = await User.findById(req.user._id); //finding user so that he can update

    const { name, email, password, skills, about } = req.body;   //need them from req.body (model ke user.js files k objects)
    ...
    if (skills) {
      if (skills.image1) {
        await cloudinary.v2.uploader.destroy(user.skills.image1.public_id);   //to delete previous data so that it dont take space in cloudinary
        const myCloud = await cloudinary.v2.uploader.upload(skills.image1, {  //to upload new data
          folder: "portfolio",
        });
